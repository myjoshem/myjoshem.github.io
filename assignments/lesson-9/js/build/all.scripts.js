...
const section = document.querySelector('section.town-data');
...
request.onload = function() {
  const townDataText = request.response; // creating variable depending on response type [js object with 'json'], [text string with 'text' response]
  ...
  const townData = JSON.parse(townDataText);
...
function populateTownData(jsonObj) {
  const towns = jsonObj.towns;// populating a new array with our town data
  const arrayTowns = ['Preston', 'Soda Springs', 'Fish Haven'];
